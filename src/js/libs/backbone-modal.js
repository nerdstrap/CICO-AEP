    ...
    Backbone.ModalView = Backbone.View.extend(
        {
            ...
            recentre: // Re-centre the modal dialog after it has been displayed. Useful if the height changes after initial rendering eg; jquery ui tabs will hide tab sections
                function()
                {
                    ...
                    modalContainer.css({"top": Math.max(1,(positionY + offsets.y)) + "px"});
            ...
            showModal:
                function(options)
                {
                    ...
                    {
                        if (coords.top != null)
                            modalContainer.css({"top": coords.top + offsets.y});
                        if (coords.left != null)
                            modalContainer.css({"left": coords.left + offsets.x});
                        if (coords.right != null)
                            modalContainer.css({"right": coords.right});
                        if (coords.bottom != null)
                            modalContainer.css({"bottom": coords.bottom});
                    }
